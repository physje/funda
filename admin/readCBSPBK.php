...
if($type == 'regio') {
  ...
  $url = "https://opendata.cbs.nl/ODataApi/odata/83906NED/UntypedDataSet";
...
for($i=1 ; $i <= $numberItems ; $i++) {
	$slice = array_slice($aPBK["value"], (($deel*$i)-4), 3);
...
foreach ($aPBK["value"] as $rij) {
  ...
  if(strpos($Periode, 'MM') AND $type == 'alles') {
    ...
    $doorgaan = true;
	} elseif(strpos($Periode, 'KW') AND ($type == 'regio' OR $type == 'woning')) {
		$jaar = substr($Periode, 0, 4);
	...
	if($type == 'alles' AND $doorgaan) {
		$groep	    = 'totaal';
    $regio	    = 'Totaal';
		$comment    = $jaar.$maand.', Totaal';
  } elseif($type == 'regio' AND $doorgaan) {
  ...
  } elseif($type == 'woning' AND $doorgaan) {
		$groep	= 'woningtype';
		$regio	= $woningtype;
		$comment	= $jaar.'Q'.$kwartaal.', '. $woningtype;
  }
  
  if($type == 'woning' AND ($TypeWoning == 'T001100' OR $PBK == '.')) {
  	$doorgaan = false;
  }
